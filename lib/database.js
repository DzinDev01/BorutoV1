  ...
  DataBase = class mongoDB {
      ...
      this.url = url
			...
			mongoose.connection.on('disconnected', async () => {
			  if (this.isReconnecting) return
				...
				console.warn('❗ Koneksi MongoDB terputus. Mencoba menyambung kembali dalam 5 detik...');
    ...
    connect = async (retries = 5, delay = 2000) => {
      ...
      this.isConnecting = true
			while (retries > 0) {
			  try {
			    console.log(`🔄 Mencoba untuk terhubung mongoDB... (Percobaan ${6 - retries}/5)`)
					...
					console.log('✅ Berhasil terhubung mongoDB.')
					this.isConnecting = false
					this.isReconnecting = false
					return
			  } catch (err) {
